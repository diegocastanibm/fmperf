...
if __name__ == "__main__":
    ...
    WORKLOAD_FILE = os.path.join(os.path.dirname(__file__), "lmbench_vllm-raw_workload_spec.yaml")
    ...
    stack_spec = StackSpec(
        ...
        stack_type="vllm-raw",  # This will automatically set endpoint to vllm-router-service
        refresh_interval=300,  # Refresh model list every 5 minutes
        # endpoint_url="llama-3-8b"  # Service name
        endpoint_url="llama-3-8b.e2e-solution.svc.cluster.local",  # Service name
        router_port= 80
    )
